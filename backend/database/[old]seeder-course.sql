-- =====================================================
-- DELTA INDONESIA 2026 TRAINING SCHEDULE
-- Based on JADWAL PUBLIC TRAINING DIP 2026.xlsx
-- =====================================================

-- Insert 2026 courses (only if not exist)
INSERT INTO courses (
  code, name, category, certification, duration_value, duration_unit,
  description, objectives, price_regular, currency, featured, status
)
SELECT * FROM (VALUES
  ...
  ('KEM-P2K3-001', 'P2K3 Panitia Pembina K3', 'kemnaker', 'Kementerian Ketenagakerjaan RI', 3, 'days',
   ...
   4080000, 'IDR', FALSE, 'active')
) AS t(code, name, category, certification, duration_value, duration_unit, description, objectives, price_regular, currency, featured, status)
WHERE NOT EXISTS (
...
SELECT 
  ...
  SUM(price_regular) as total_revenue_potential,
  AVG(price_regular) as average_price
FROM courses 
WHERE code LIKE 'KEM-%' AND created_at > NOW() - INTERVAL '1 hour';
