...
BEGIN
    ...
    FOR monthNum IN 1..12 LOOP
        ...
        IF courseRec.id IS NOT NULL THEN
            ...
            IF startDay IS NOT NULL AND endDay IS NOT NULL THEN
                -- Handle cross-month schedules
                IF endDay < startDay THEN
                    -- Cross-month: start in current month, end in next month
                    startDate := make_date(2026, monthNum, startDay);
                    IF monthNum = 12 THEN
                        endDate := make_date(2027, 1, endDay);
                    ELSE
                        endDate := make_date(2026, monthNum + 1, endDay);
                    END IF;
                ELSE
                    -- Normal: both dates in same month
                    startDate := make_date(2026, monthNum, startDay);
                    endDate := make_date(2026, monthNum, endDay);
                END IF;
...
SELECT
    ...
    MIN(cs."startDate") as firstBatch,
    MAX(cs."startDate") as lastBatch
FROM courses c LEFT JOIN "courseSchedules" cs ON c.id = cs."courseId"
