...
const MediaInfo = () => {
  ...
  // Separate featured and media release items
  const featuredItem = mediaItems.find((item) => item.type === "featured");
  const mediaReleaseItems = mediaItems.filter(
    (item) => item.type === "media-release"
  );

  // Calculate max slides based on media release items
  const maxSlides = Math.max(0, mediaReleaseItems.length - 2);
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* FEATURED CARD - Always Pinned (Position 1) */}
            {featuredItem && (
              <div className="md:col-span-1 md:row-span-2 relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all group bg-white z-30">
            ...
            {/* MEDIA RELEASE CARDS - Scrollable (Position 2 & 3) */}
            {mediaReleaseItems
              .slice(currentIndex, currentIndex + 2)
              .map((item, index) => (
                <div
                  key={index}
                  className={`md:col-span-1 relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all group bg-white ${
                    index === 0 ? "z-10" : "z-20"
                  }`}
                >
                  {/* Card */}
